...
public class QuoteService {
    ...
    // 🔹 Listar solo las cotizaciones inactivas
    public Flux<Quote> getAllInactiveQuotes() {
        System.out.println("🔍 Servicio: Buscando quotes inactivos");
        return quoteRepository.findAll()
                .doOnNext(quote -> System.out.println("📋 Quote encontrado: ID=" + quote.getId() + ", Status='" + quote.getStatus() + "'"))
                .filter(quote -> {
                    boolean esInactivo = "inactivo".equalsIgnoreCase(quote.getStatus());
                    System.out.println("🔎 ID=" + quote.getId() + " ¿Es inactivo? " + esInactivo);
                    return esInactivo;
                });
    }
