...
public class VehicleController {
    ...
    return vehicleService.createVehicle(dto)
        .doOnSuccess(created -> log.info("✅ Vehicle created successfully: ID={}, Model={}", created.getId(), created.getModelo()))
        .flatMap(created -> org.springframework.security.core.context.ReactiveSecurityContextHolder.getContext()
            ...
            .flatMap(a -> {
                ...
                return notificationService.createNotification("vehicles", created.getId() != null ? Long.valueOf(created.getId()) : null, "CREATE", "Vehículo creado: " + created.getModelo(), actorId, actorEmail).thenReturn(created);
            })
        )
    ...
    return vehicleService.updateVehicle(id, dto)
        .doOnSuccess(updated -> log.info("✅ Vehicle updated successfully: ID={}, Model={}", updated.getId(), updated.getModelo()))
        .flatMap(updated -> org.springframework.security.core.context.ReactiveSecurityContextHolder.getContext()
            ...
            .flatMap(a -> {
                ...
                return notificationService.createNotification("vehicles", updated.getId() != null ? Long.valueOf(updated.getId()) : null, "UPDATE", "Vehículo actualizado: " + updated.getModelo(), actorId, actorEmail).thenReturn(updated);
            })
        )
        .map(ResponseEntity::ok)
        .defaultIfEmpty(ResponseEntity.notFound().build())
        ...
        return vehicleService.updateEstado(id, estado)
                .then(org.springframework.security.core.context.ReactiveSecurityContextHolder.getContext()
                    ...
                    .flatMap(a -> {
                        ...
                        return notificationService.createNotification("vehicles", Long.valueOf(id), "UPDATE_STATUS", "Estado de vehículo actualizado a: " + estado, actorId, actorEmail);
                    })
                )
    ...
    public Mono<ResponseEntity<String>> testNotification() {
        ...
        try {
            return notificationService.createNotification("vehicles", 999L, "TEST", "Esta es una notificación de prueba", 1L, "test@example.com")
                    .map(notification -> {
